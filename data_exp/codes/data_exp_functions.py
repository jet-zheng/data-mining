...
def main():
    ...
    #将异常值和相关系数打印到pdf
    doc = SimpleDocTemplate('C:\\Report_File.pdf')
    doc = dataframe_to_pdf([corr_res, outlier_res], ["-------------------------CORR-------------------------", "-------------------------OUTLINER-------------------------"], doc)
...
def get_outlier(df_input,method='zscore',outlier_threshod=3):
    ...
    # 返回只有异常值得列
    return_data = pd.DataFrame([series for series in outlier if(isinstance(series,pd.Series))])
    return_data.columns = [""]
...
def compute_outlier_withzscore(series,outlier_threshod):
    ...
    if(outlier_series.size >0):
        return pd.Series([outlier_series]).rename(series.name)
...
def dataframe_to_pdf(df_input,titles,doc):
    ...
    for index in range (len(df_input)):
        ...
        while(read_postition<clen):
            ...
            if(read_postition <(clen-step)):
                pdf_df = df_input[index].ix[:,read_postition:read_postition+step]
            else:
                pdf_df = df_input[index].ix[:, read_postition:clen]
